...
//
 gulp.task('replaceName', function () {
    return gulp.src('../public/javascript/pdf2csv/components/components/*.html')
     ...
     .pipe(gulp.dest('../public/javascript/pdf2csv/components/components'));
 });

gulp.task('angularMerge', function () {
  return gulp.src('../public/javascript/pdf2csv/components/*.js')
    .pipe(embedTemplates({ sourceType: 'js', minimize: {quotes: true, empty: true } }))
    .pipe(gulp.dest('../public/javascript/pdf2csv/components/'));
});

gulp.task('clean', function () {
  return gulp.src('../public/javascript/pdf2csv/components/components')
